...
1. 準確預測個體在未來時間點（T3）罹患三高疾病的風險
2. 利用時間序列資訊（T1→T2 的變化）提升預測性能
3. 提供可解釋的預測結果，支持臨床決策
...
### 假設 4: 特徵重要性排序
**H4**: 基線血糖（FBG_T2）和血糖變化量（Δ FBG）將是高血糖預測的前兩大重要特徵；類似地，血壓相關特徵對高血壓、血脂相關特徵對高血脂具有最高重要性。
...
**時間點 (Time Point)**
- T1: 基線（Baseline），第一次健檢
- T2: 追蹤（Follow-up），第二次健檢
- T3: 預測目標（Target），第三次健檢

**變化量特徵 (Delta Features / Δ Features)**
- 定義：Δ = T2 - T1
- 捕捉健康狀態的動態變化
- 例：Δ FBG = FBG_T2 - FBG_T1
