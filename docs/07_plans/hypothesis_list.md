...
**結論**：✅ 能，Sensitivity 從 4~34% 提升到 70~88%

### 完整比較結果（Notebook 24）

| 方法 | 高血壓 Sens | 高血糖 Sens | 高血脂 Sens | 狀態 |
|------|------------|------------|------------|------|
| Baseline | 0.041 | 0.335 | 0.135 | ✅ |
| class_weight | 0.698 | 0.861 | 0.791 | ✅ |
| SMOTE | 0.698 | 0.852 | 0.785 | ✅ |
| ADASYN | 0.696 | **0.877** | **0.794** | ✅ |
| UnderSampling | **0.699** | 0.864 | 0.790 | ✅ |

### 關鍵發現

1. **AUC 完全不受影響**：所有方法的 AUC 差異 < 0.2%
2. **Sensitivity 大幅提升**：從 Baseline 提升 +50~60%
3. **方法間差異極小**：class_weight、SMOTE、ADASYN、UnderSampling 效果幾乎相同（差異 < 2%）

### 結論

**SMOTE 不比 class_weight 更好**，推薦使用 `class_weight='balanced'`：
- 最簡單（不需額外套件）
- 效果相同
- 不改變資料分佈
