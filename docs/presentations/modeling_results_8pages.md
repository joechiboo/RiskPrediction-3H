...
| **部署成本** | 1 個模型 vs 3 個 | 差異不大，都很輕量 |
| **效能提升** | 共享表徵學習 | RF MTL 高血糖 +0.022，**其他無顯著提升** |

**反思**: 當初選擇 MTL 是預期能減少計算時間，但實際資料量僅 6,056 筆，傳統模型（LR, RF, XGBoost）都是秒級完成，MTL 的效率優勢**並未體現**。
...
**原因**：
1. **效率優勢不存在** - 資料量小，Single-Task 也是秒級完成
2. **效能提升有限** - 僅高血糖有 +0.022 提升，其他無顯著差異
3. **增加複雜度** - ANN MTL 需要調整 loss_weights，容易失敗
4. **Single-Task 已足夠好** - LR 單任務在高血糖/高血脂已達最佳

**建議方向**：
- 回歸 Single-Task 模型，各疾病選用最佳模型
- MTL 保留作為學術探索的嘗試記錄
- 若未來資料量達百萬級，可重新考慮 MTL

### 📢 講稿 (1分鐘)

接下來談談Multi-Task Learning的嘗試與反思。當初選擇MTL是因為我們需要同時預測三種疾病,預期能透過共享表徵學習減少計算時間。但實際情況是,我們的資料量只有6056筆,所有傳統模型都是秒級完成,MTL的效率優勢並沒有體現出來。在效能方面,Random Forest的MTL版本確實在高血糖上提升了0.022,從第五名躍升到第二名,但其他疾病沒有顯著提升。ANN的MTL版本更是完全失敗,因為類別不平衡導致少數疾病被忽略。綜合評估後,我們認為未來可能會捨棄MTL,原因有三:第一,效率優勢不存在;第二,效能提升有限;第三,Single-Task模型已經足夠好,比如Logistic Regression在高血糖和高血脂都是冠軍。這次MTL的嘗試雖然結果不如預期,但作為學術探索仍有價值,也讓我們更清楚地認識到方法選擇需要根據實際數據特性來決定。
