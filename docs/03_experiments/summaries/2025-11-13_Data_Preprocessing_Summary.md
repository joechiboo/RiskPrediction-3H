...
### 訓練/測試分割
```
訓練階段：Y-2 + Δ1 → 預測 Y-1 的疾病狀態
測試階段：Y-1 + Δ2 → 預測 Y 的疾病狀態
```

### 特徵組合方案
1. **基礎模型**：只用 Y-2 特徵（8項）
2. **加入變化量**：Y-2 + Δ1（16項）
3. **完整時序**：Y-2 + Y-1 + Δ1（24項）
...
```python
# Δ1 = Y-1 − Y-2
df_wide['ΔFBG'] = df_wide['FBG_Y-1'] - df_wide['FBG_Y-2']

# Δ2 = Y − Y-1
df_wide['ΔFBG'] = df_wide['FBG_Y'] - df_wide['FBG_Y-1']
```
...
4. **資料結構清晰**
   ...
   - 每人一筆記錄，避免資料洩漏
   - 欄位命名規則統一（feature_Y-2, feature_Y-1...）
...
**Q: 建模策略是什麼？**
**A**: 訓練用 Y-2+Δ1 預測 Y-1，測試用 Y-1+Δ2 預測 Y
