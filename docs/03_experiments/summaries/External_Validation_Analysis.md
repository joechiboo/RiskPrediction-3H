...
### 5.2 外部驗證有效性討論（待與教授確認）

#### 理想的外部驗證條件

1. **相同的 population**：年齡、性別、疾病盛行率相近
2. **相同的特徵**：所有模型使用的特徵都可取得
3. **不同的資料來源**：來自不同機構或時間
...
| 項目 | SUA | Synthea | 問題 |
|------|-----|---------|------|
| 平均年齡 | 68 歲 | 44 歲 | 完全不同的族群 |
| 高血壓盛行率 | 19% | 42% | 盛行率差 2 倍 |
| 資料性質 | 真實健檢 | 合成資料 | 特徵關係可能不同 |
| 可用特徵 | 26 個 | 11 個 | 缺少實驗室數據 |

#### 核心問題

**年齡差異導致盛行率不同，這樣的外部驗證是否有意義？**

- 這不是「相同模型在不同資料上的表現」
- 而是「針對老年人訓練的模型用在不同年齡層上」
- AUC 下降可能反映的是「族群差異」而非「模型泛化能力」

#### 建議與教授討論

1. **是否將此分析納入論文？**
   - 選項 A：納入為「有限度的外部驗證」
   - 選項 B：改為「研究限制」說明為何無法做有效外部驗證

2. **若納入，如何詮釋 AUC 下降？**
   - 是「模型泛化能力不足」？
   - 還是「族群差異導致的預期下降」？

3. **是否需要申請 MJ Health 資料？**
   - 可取得相同族群的真實資料
   - 但需要時間和審核流程
