...
| 疾病 | 公式 | 醫學意義 |
|------|------|----------|
| **高血壓** | `0.106 × exp(SBP_Y-2)` | ✅ 收縮壓越高，風險指數增長 |
| **高血糖** | `0.116 × FBG_Y-1` | ✅ 空腹血糖越高，風險越高 |
| **高血脂** | `0.052 × exp(TC_Y-2)` | ✅ 總膽固醇越高，風險指數增長 |
...
|------|-----|----------|------|-------|--------|
| 高血壓 | 0.745 | **10.8 分鐘** | `0.106×exp(SBP_Y-2)` | 138/64 | 68.3% |
| 高血糖 | 0.943 | **10.1 分鐘** | `0.116×FBG_Y-1` | 72/6 | 92.3% |
| 高血脂 | 0.801 | **10.5 分鐘** | `0.052×exp(TC_Y-2)` | 65/8 | 89.0% |
| **總計** | - | **~31 分鐘** | - | - | - |
...
|------|------------|------------|------|------|
| 高血壓 | 0.745 | 0.745 | 無變化 | `0.130 × exp(SBP_Y-2)` |
| 高血糖 | 0.943 | 0.943 | 無變化 | `0.114 × FBG_Y-1` |
| 高血脂 | 0.801 | 0.801 | 無變化 | `0.043 × exp(TC_Y-2)` |
...
**原因分析**：
1. **高血糖**：`FBG_Y-1` 訊號強，40 次就找到
2. **高血壓**：需要 `exp(SBP_Y-2)` 非線性關係，搜尋空間更大
3. **教授建議正確**：增加 generations 對複雜問題有效

### 4.2 公式的醫學意義

**高血壓**：`0.106 × exp(SBP_Y-2)`
- SBP（收縮壓）是高血壓的直接指標
- 使用指數函數：高血壓風險隨 SBP 非線性增長
- 與 SHAP 分析一致：SBP_Y-2 是最重要特徵
...
```text
高血壓風險 = 0.106 × exp(SBP_Y-2)    # AUC=0.745
高血糖風險 = 0.116 × FBG_Y-1         # AUC=0.943 ⭐
高血脂風險 = 0.052 × exp(TC_Y-2)     # AUC=0.801
```
...
### 6.1 滑動窗口 5-Fold CV 結果（最新）

| 疾病 | PySR AUC | 穩定性 | 公式 | LR (5-Fold) |
|:----:|:--------:|:------:|:-----|:-----------:|
| 高血壓 | 0.580 ±0.110 | ❌ 3/5 常數 | `0.09*SBP_Y-1 + 0.19` | 0.721 |
| **高血糖** | **0.918 ±0.016** | ✅ 5/5 穩定 | `0.12*FBG_Y-1` | 0.938 |
| 高血脂 | 0.640 ±0.192 | ❌ 3/5 常數 | `0.09*TC_Y-1 + 0.08` | 0.867 |
...
| 發現 | 說明 |
|:-----|:-----|
| 高血糖公式穩定 | 5/5 fold 都產出 `0.12*FBG_Y-1`，AUC=0.918 接近 LR |
| 高血壓/高血脂失敗 | 3/5 fold 退化為常數解（AUC≈0.5） |
| PySR 定位 | **可解釋性補充工具**，非主力模型 |
...
目前三個公式都是**單一特徵**：
```text
高血壓：0.106 × exp(SBP_Y-2)      # 深度=1
高血糖：0.116 × FBG_Y-1           # 深度=1
高血脂：0.052 × exp(TC_Y-2)       # 深度=1
```
...
```text
# 期望的更複雜公式
高血壓：a × SBP_Y-2 + b × exp(Age) + c × BMI_Y-1
高血糖：a × FBG_Y-1 × (1 + b × BMI_Y-2)
高血脂：a × TC_Y-2 + b × TG_Y-1 / HDL_Y-2
```
