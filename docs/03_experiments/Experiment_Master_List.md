...
**最佳公式示例**：
- 高血壓：`abs((|Delta_SBP| + SBP_Y-2 + 1.46) * 0.15)`（AUC=0.780）
- 高血糖：`0.12 * FBG_Y-1`
- 高血脂：`0.04 * exp(TC_Y-2)`
...
| **結論** | 高血糖成功（AUC 0.918，公式 `0.12*FBG_Y-1`）；高血壓/高血脂不穩定（3/5 fold 退化常數） |

**結果**：

| 疾病 | AUC | 公式穩定性 | 代表公式 |
|------|-----|-----------|----------|
| 高血壓 | 0.580 ± 0.110 | ❌ 3/5 常數 | `0.09*SBP_Y-1 + 0.19` |
| 高血糖 | 0.918 ± 0.016 | ✅ 5/5 穩定 | `0.12*FBG_Y-1` |
| 高血脂 | 0.640 ± 0.192 | ❌ 3/5 常數 | `0.09*TC_Y-1 + 0.08` |
...
- depth=1 `0.10*SBP_Y-1` → AUC ≈ 0.70
- depth=2 `0.065*(SBP_Y-2+SBP_Y-1)` → AUC ≈ 0.71（僅 +1%）

**建議**：論文以高血糖公式 `0.12*FBG_Y-1`（AUC=0.918）為主要範例
