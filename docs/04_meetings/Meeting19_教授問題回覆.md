...
| # | 任務 | 狀態 | 對應文件/Notebook |
|---|------|------|-------------------|
| 1 | MJ Health 資料申請 | ✅ 整理完成 | [MJ_Health_資料申請指南.md](../01_data/MJ_Health_資料申請指南.md) |
| 2 | Problem Definition | ✅ 完成 | [Problem_Definition.md](../07_plans/Problem_Definition.md) |
| 3 | 選讀論文 | ✅ 已選定 | Kanegae 2020（待做簡報） |
...
| 7 | PySR 深度實驗 | ⏳ 進行中 | Notebook 23 |
| 8 | 健檢次數實驗 | ✅ 完成 | Notebook 14 |
| 9 | 5-Fold CV | ✅ 完成 | Notebook 17 |
| 10 | 輸出是機率還是二元？ | ⏳ 待釐清 | 見下方說明 |
...
模型輸出到底是：
- **風險機率** (0~1 的連續值，如 0.73)
- **是/否** (二元分類，如 1 或 0)

### 目前做法

```
模型輸出 → predict_proba() → 機率值 (0~1)
                ↓
        閾值 (threshold=0.5)
                ↓
            是/否 (0 or 1)
...
| 輸出類型 | 用途 | 評估指標 |
|----------|------|----------|
| **機率** | 風險排序、醫療決策參考 | AUC-ROC |
| **二元** | 明確分類、篩檢 | Accuracy, Sensitivity, Specificity |

### 待確認

1. 論文中要強調哪一種輸出？
2. 臨床應用上，醫生偏好看機率還是是/否？
3. 是否需要調整閾值（如 0.3）來提高 Sensitivity？
