...
We utilized a publicly available longitudinal dataset (Luo et al., 2023) comprising 6,056 adults aged 40+ followed from 2010 to 2018. A three-timepoint design (Y-2, Y-1, Y0) was adopted, using health indicators and delta features (Δ) to predict disease status. A sliding window approach generated 13,514 records. Ten models (traditional statistics, KNN, tree-based, kernel methods, and neural network) were compared, with symbolic regression exploring interpretability. StratifiedGroupKFold 5-fold cross-validation ensured no data leakage.
