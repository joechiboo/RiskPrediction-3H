...
支援向量機（Support Vector Machine, SVM）由 Cortes & Vapnik (1995) 提出，基於統計學習理論，透過尋找最大間隔超平面進行分類，並可使用核函數處理非線性問題。本研究採用 SVM 作為核方法（Kernel Method）的代表，與線性方法（LR、NB、LDA）、樹模型（DT、RF、XGBoost）及神經網路（MLP）形成四類方法的完整比較架構。
