...
2. **T1 vs T2 哪個時間點更重要？** - 比較 T1-Only vs T2-Only
...
| 特徵集 | 包含特徵 | 特徵數 |
|--------|----------|--------|
| **Full** | sex, Age, T1, T2, Δ | 26 |
| **No-Δ** | sex, Age, T1, T2 | 18 |
| **Δ-Only** | Δ | 8 |
| **T1-Only** | sex, Age, T1 | 10 |
| **T2-Only** | sex, Age, T2 | 10 |
| **T1+Δ** | sex, Age, T1, Δ | 18 |
| **T2+Δ** | sex, Age, T2, Δ | 18 |
...
### 3.4 T1 vs T2 vs Δ 比較

| 疾病 | 模型 | T1-Only | T2-Only | Δ-Only | 最佳 |
|------|------|---------|---------|--------|------|
| 高血壓 | LR | 0.748 | 0.705 | 0.539 | **T1** |
| 高血壓 | XGB | 0.736 | 0.681 | 0.585 | **T1** |
| 高血糖 | LR | 0.864 | 0.926 | 0.770 | **T2** |
| 高血糖 | XGB | 0.827 | 0.911 | 0.717 | **T2** |
| 高血脂 | LR | 0.825 | 0.845 | 0.613 | **T2** |
| 高血脂 | XGB | 0.785 | 0.825 | 0.558 | **T2** |

**關鍵發現**：
- **高血壓**：T1 特徵更重要（初始血壓是關鍵預測因子）
- **高血糖/高血脂**：T2 特徵更重要（最近的檢測值更能預測未來風險）
- **Δ-Only**：效能最低（缺乏絕對值資訊）
...
| 發現 | SHAP 分析 | 消融實驗 |
|------|-----------|----------|
| 高血壓主導特徵 | SBP_T1 最重要 | T1-Only > T2-Only ✓ |
| 高血糖主導特徵 | FBG_T2 最重要 | T2-Only > T1-Only ✓ |
| 高血脂主導特徵 | TC_T2 最重要 | T2-Only > T1-Only ✓ |
...
| 情境 | 建議 |
|------|------|
| 只有單次健檢 | 優先使用最近一次（T2 等效） |
| 有多次健檢 | 使用 Full 特徵集（T1+T2+Δ） |
| 計算資源有限 | 可捨棄 Δ（損失 <2%） |
| 高血壓預測 | 重視 T1（初始血壓）|
| 高血糖/高血脂預測 | 重視 T2（最近檢測值）|

### 5.3 研究意義

- **驗證了 Δ 特徵的價值**：雖然貢獻有限，但對某些疾病有統計顯著的提升
- **解釋了 T1 vs T2 的差異**：與 SHAP 分析結果一致
...
| `delta_t1_t2_comparison.png` | T1 vs T2 vs Δ 比較圖 |
