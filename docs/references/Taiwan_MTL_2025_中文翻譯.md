...
**健康與福利資料科學中心（HWDC）簡介**：
- **全名**：Health and Welfare Data Science Center
- **隸屬單位**：衛生福利部
- **位置**：台北南港
- **成立目的**：整合並提供台灣健保資料庫供學術研究使用
- **資料規模**：從全民健保 2,300 萬人中隨機抽樣 200 萬人（約 8.7% 樣本）
- **資料期間**：涵蓋多年度連續性醫療紀錄
- **資料特色**：
...
**本研究使用的資料子集**：
- 從 200 萬人樣本中篩選符合條件的患者
- 回顧性收集（Retrospective Study）
- 最終納入 555,124 筆有效樣本

#### 倫理審查與資料保護

**研究倫理批准**：
- **審查機構**：國立台灣大學研究倫理委員會（NTU-REC）
- **核准編號**：202108HM002
- **批准日期**：2021 年 8 月 9 日
- **審查類型**：人體研究倫理審查

**知情同意豁免**：
- 由於本研究為回顧性研究，使用去識別化的既有資料
- 不涉及人體介入或直接接觸受試者
- 符合研究倫理豁免知情同意的條件
- 國立台灣大學研究倫理委員會正式豁免知情同意要求

**倫理準則遵循**：
- 遵循《赫爾辛基宣言》（Declaration of Helsinki）的倫理指南
- 確保資料使用僅限於學術研究目的
- 嚴格保護患者隱私與資料安全
- 研究結果不包含任何可識別個人身份的資訊
...
**預測目標**：
使用患者過去 10 年的個人資訊和醫療紀錄，預測未來 5 年內以下四種慢性疾病的發生風險：
...
**時間軸設計**：
- **回溯期**：過去 10 年（T₀ - 10 年 至 T₀）
- **預測期**：未來 5 年（T₀ 至 T₀ + 5 年）
- **資料截點**：T₀（當前時間點）
...
**1. 受益人登記檔（Registry for Beneficiaries）**
- **用途**：提取患者基本個人資訊
- **關鍵欄位**：
  - `ID`：患者唯一識別碼（去識別化）
  - `ID_BIRTH_Y`：出生年份
...
**2. 門診醫療支出檔（Ambulatory Care Expenditures）**
- **用途**：提取患者就醫紀錄與診斷資訊
- **關鍵欄位**：
  - `ID`：患者唯一識別碼（與受益人檔合併）
  - `AGE`：就醫時年齡
  - `FEE_YM`：就醫年月（YYYYMM 格式）
  - `ICD_CM`：國際疾病分類代碼（ICD-9-CM 或 ICD-10-CM）
  - 診斷代碼、處方資訊、醫療費用等

**資料處理步驟**：
1. **年齡計算**：從 `ID_BIRTH_Y` 欄位計算患者當前年齡
2. **疾病識別**：從 `ICD_CM` 欄位提取診斷的 ICD 代碼
3. **時間序列建構**：從 `FEE_YM` 欄位推導就診時間，建立時序特徵
4. **資料合併**：透過 `ID` 欄位合併個人資訊與醫療紀錄
5. **特徵工程**：將原始資料轉換為模型可用的特徵向量
