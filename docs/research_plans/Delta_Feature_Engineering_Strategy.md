...
```
患者 ID | T1 日期 | T1 FPG | T2 日期 | T2 FPG | T3 日期 | T3 FPG
--------|---------|--------|---------|--------|---------|--------
...
```python
# 短期變化 (T2-T1)
Δ_FPG_T2_T1 = FPG_T2 - FPG_T1

# 短期變化 (T3-T2)
Δ_FPG_T3_T2 = FPG_T3 - FPG_T2

# 長期變化 (T3-T1)
Δ_FPG_T3_T1 = FPG_T3 - FPG_T1
```
...
```python
# 變化率 = 變化量 / 時間間隔
變化率_T2_T1 = (FPG_T2 - FPG_T1) / (時間_T2 - 時間_T1)
變化率_T3_T1 = (FPG_T3 - FPG_T1) / (時間_T3 - 時間_T1)
...
```python
# 相對變化 = (新值 - 舊值) / 舊值 × 100%
相對變化_T2_T1 = (FPG_T2 - FPG_T1) / FPG_T1 × 100
相對變化_T3_T1 = (FPG_T3 - FPG_T1) / FPG_T1 × 100
```
...
#### **1. 血糖相關**
```python
Δ_FPG = FPG_T2 - FPG_T1          # 空腹血糖變化
Δ_HbA1c = HbA1c_T2 - HbA1c_T1    # 糖化血色素變化（如有）
```

#### **2. 尿酸相關** ⭐ 本研究核心
```python
Δ_SUA = SUA_T2 - SUA_T1          # 血清尿酸變化
Δ_BUN = BUN_T2 - BUN_T1          # 尿素氮變化
Δ_Creatinine = Cr_T2 - Cr_T1     # 肌酸酐變化
```

#### **3. 血脂相關**
```python
Δ_TG = TG_T2 - TG_T1             # 三酸甘油酯變化
Δ_LDL = LDL_T2 - LDL_T1          # 低密度脂蛋白變化
Δ_HDL = HDL_T2 - HDL_T1          # 高密度脂蛋白變化
Δ_TC = TC_T2 - TC_T1             # 總膽固醇變化
```

#### **4. 肝功能相關**
```python
Δ_GOT = GOT_T2 - GOT_T1          # 麩胺酸苯醋酸轉胺酶變化
Δ_GPT = GPT_T2 - GPT_T1          # 麩胺酸丙酮酸轉胺酶變化
Δ_γGT = γGT_T2 - γGT_T1          # γ-麩胺酸轉移酶變化
```

#### **5. 體重/代謝相關**
```python
Δ_BMI = BMI_T2 - BMI_T1          # 身體質量指數變化
Δ_體脂肪 = 體脂肪_T2 - 體脂肪_T1  # 體脂肪變化（如有）
Δ_腰圍 = 腰圍_T2 - 腰圍_T1       # 腰圍變化（如有）
```
...
#### **7. 發炎/血液相關**
```python
Δ_WBC = WBC_T2 - WBC_T1          # 白血球計數變化
Δ_CRP = CRP_T2 - CRP_T1          # C反應蛋白變化（如有）
Δ_Hb = Hb_T2 - Hb_T1             # 血紅素變化
Δ_Platelet = Plt_T2 - Plt_T1     # 血小板變化
```

#### **8. 其他代謝指標**
```python
Δ_TSH = TSH_T2 - TSH_T1          # 促甲狀腺激素變化（如有）
Δ_LDH = LDH_T2 - LDH_T1          # 乳酸脫氫酶變化（如有）
Δ_ALP = ALP_T2 - ALP_T1          # 鹼性磷酸酶變化（如有）
```
...
```
患者 P001：
T1 (2020): FPG = 88 mg/dL  ✅ 正常
T2 (2021): FPG = 94 mg/dL  ✅ 正常
T3 (2022): FPG = 102 mg/dL ⚠️ 前驅糖尿病

計算 Δ 特徵：
Δ_FPG_T2_T1 = 94 - 88 = +6 mg/dL
Δ_FPG_T3_T2 = 102 - 94 = +8 mg/dL
Δ_FPG_T3_T1 = 102 - 88 = +14 mg/dL  🚨 總上升 14
...
```
患者 P002：
T1 (2020): FPG = 90 mg/dL  ✅ 正常
T2 (2021): FPG = 98 mg/dL  ⚠️ 接近閾值
T3 (2022): FPG = 92 mg/dL  ✅ 正常

計算 Δ 特徵：
Δ_FPG_T2_T1 = 98 - 90 = +8 mg/dL   ⚠️ 上升
Δ_FPG_T3_T2 = 92 - 98 = -6 mg/dL   💚 下降（可能開始運動減重）
Δ_FPG_T3_T1 = 92 - 90 = +2 mg/dL   ✅ 總體穩定
...
```
患者 P003：
T1 (2020): FPG = 95, SUA = 6.5, TG = 120, BMI = 24
T2 (2021): FPG = 98, SUA = 7.2, TG = 150, BMI = 25.5
...
```python
# 模型 A：僅使用基線特徵 (T1)
features_A = [FPG_T1, SUA_T1, TG_T1, BMI_T1, ...]

# 模型 B：僅使用 Δ 特徵
features_B = [Δ_FPG, Δ_SUA, Δ_TG, Δ_BMI, ...]

# 模型 C：基線 + Δ 特徵
features_C = [FPG_T1, SUA_T1, ..., Δ_FPG, Δ_SUA, ...]
...
for col in continuous_features:
    # 兩兩變化
    df[f'Δ_{col}_T2_T1'] = df[f'{col}_T2'] - df[f'{col}_T1']
    df[f'Δ_{col}_T3_T2'] = df[f'{col}_T3'] - df[f'{col}_T2']
    df[f'Δ_{col}_T3_T1'] = df[f'{col}_T3'] - df[f'{col}_T1']

    # 變化率
    df[f'Rate_{col}_T2_T1'] = df[f'Δ_{col}_T2_T1'] / df['時間差_T2_T1']
    df[f'Rate_{col}_T3_T1'] = df[f'Δ_{col}_T3_T1'] / df['時間差_T3_T1']

    # 相對變化
    df[f'RelΔ_{col}_T2_T1'] = df[f'Δ_{col}_T2_T1'] / df[f'{col}_T1'] * 100
    df[f'RelΔ_{col}_T3_T1'] = df[f'Δ_{col}_T3_T1'] / df[f'{col}_T1'] * 100

    delta_features.extend([f'Δ_{col}_T2_T1', f'Δ_{col}_T3_T2', f'Δ_{col}_T3_T1'])
```

**任務 1.3：目標變量定義**
```python
# 高尿酸血症
df['高尿酸'] = (df['SUA_T3'] >= 7.0).astype(int)  # 男性標準

# 前驅糖尿病
df['前驅糖尿病'] = ((df['FPG_T3'] >= 100) & (df['FPG_T3'] < 126)).astype(int)

# 糖尿病
df['糖尿病'] = (df['FPG_T3'] >= 126).astype(int)

# 高血脂
df['高血脂'] = (df['TG_T3'] >= 150).astype(int)
...
```python
# 特徵：T1 時間點的所有指標
features_A = [col for col in df.columns if col.endswith('_T1')]
...
```python
# Δ_SUA × Δ_BMI
shap.dependence_plot("Δ_SUA_T3_T1", shap_values, X_test,
                     interaction_index="Δ_BMI_T3_T1")
...
summary_stats = df.groupby('結果').agg({
    'SUA_T1': ['mean', 'std'],
    'Δ_SUA_T3_T1': ['mean', 'std'],
    'BMI_T1': ['mean', 'std'],
    'Δ_BMI_T3_T1': ['mean', 'std'],
    'TG_T1': ['mean', 'std'],
    'Δ_TG_T3_T1': ['mean', 'std']
})
...
for col in ['SUA_T1', 'Δ_SUA_T3_T1', 'BMI_T1', 'Δ_BMI_T3_T1']:
